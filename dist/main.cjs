// Generated by dart2js (, csp, intern-composite-values), the Dart to JavaScript compiler version: 3.14.0-956968452.
...
(function dartProgram() {
  ...
  A = {JS_CONST: function JS_CONST() {
    ...
    Version_Version$parse(text) {
      ...
      try {
        ...
        t1 = t1[1];
        t1.toString;
        major = A.int_parse(t1);
        t1 = match._match;
        if (2 >= t1.length)
          return A.ioore(t1, 2);
        t1 = t1[2];
        t1.toString;
        minor = A.int_parse(t1);
        t1 = match._match;
        if (3 >= t1.length)
          return A.ioore(t1, 3);
        t1 = t1[3];
        t1.toString;
        patch = A.int_parse(t1);
        t1 = match._match;
        if (5 >= t1.length)
          return A.ioore(t1, 5);
        preRelease = t1[5];
        ...
        t4 = preRelease;
        t5 = build;
        t4 = t4 == null || t4.length === 0 ? A._setArrayType([], type$.JSArray_Object) : A.Version__splitParts(t4);
        t5 = t5 == null || t5.length === 0 ? A._setArrayType([], type$.JSArray_Object) : A.Version__splitParts(t5);
        if (t1 < 0)
    ...
    _impl(args) {
      var $async$goto = 0,
        $async$completer = A._makeAsyncAwaitCompleter(type$.void),
        $async$returnValue, flavor, raw, os, architecture, channel, version, url, toolName, sdkPath, t2, archivePath, extractedFolder, pubCache, versionFilePath, actionPath, t1, sdk, $async$temp1;
        ...
        for (;;)
          switch ($async$goto) {
            case 0:
              // Function start
              t1 = init.G;
              sdk = A._asString(A._asJSObject(t1.core).getInput("sdk"));
              if (sdk.length === 0)
                sdk = "stable";
              if (A.Primitives_parseInt(sdk, null) != null && !B.JSString_methods.contains$1(sdk, "."))
                sdk += ".0";
              flavor = A._asString(A._asJSObject(t1.core).getInput("flavor"));
              if (flavor.length === 0)
              ...
              raw = flavor === "raw";
              os = A.getPlatform();
              architecture = A._asString(A._asJSObject(t1.core).getInput("architecture"));
              if (architecture.length === 0)
                architecture = B.JSArray_methods.contains$1(B.List_x64_ia32_arm_arm64, A._asString(A._asJSObject(t1.os).arch())) ? A._asString(A._asJSObject(t1.os).arch()) : "x64";
              channel = "stable";
            ...
            case 4:
              // join
              A._asJSObject(t1.core).info("Installing the " + os + "-" + architecture + " Dart SDK version " + version + " from the " + channel + " (" + flavor + ") channel.");
              url = string$.https_ + channel + "/" + flavor + "/" + version + "/sdk/dartsdk-" + os + "-" + architecture + "-release.zip";
              toolName = raw ? "dart_raw" : "dart";
              sdkPath = !raw ? A._asString(A._asJSObject(t1.toolCache).find(toolName, version, architecture)) : "";
              $async$goto = sdkPath.length !== 0 ? 14 : 16;
              break;
            case 14:
              // then
              A._asJSObject(t1.core).info("Using cached sdk from " + sdkPath + ".");
              // goto join
              $async$goto = 15;
              break;
            case 16:
              // else
              A._asJSObject(t1.core).info(url + " ...");
              t2 = A.Process_env(A._asJSObject(t1.process), "RUNNER_TEMP");
              t2.toString;
              archivePath = A.join(t2, A.ParsedPath_ParsedPath$parse(url, $.$get$url().style).get$basename());
              t2 = type$.String;
              $async$goto = 17;
              return A._asyncAwait(A.promiseToFuture(A._asJSObject(A._asJSObject(t1.toolCache).downloadTool(url, archivePath)), t2), $async$_impl);
            case 17:
              // returning from await.
              $async$temp1 = A;
              $async$goto = 18;
              return A._asyncAwait(A.promiseToFuture(A._asJSObject(A._asJSObject(t1.toolCache).extractZip(archivePath)), t2), $async$_impl);
            case 18:
              // returning from await.
              extractedFolder = $async$temp1.join($async$result, "dart-sdk");
              $async$goto = 19;
              return A._asyncAwait(A.promiseToFuture(A._asJSObject(A._asJSObject(t1.toolCache).cacheDir(extractedFolder, toolName, version, architecture)), t2), $async$_impl);
            case 19:
              // returning from await.
              sdkPath = $async$result;
            case 15:
              // join
              t2 = A._asJSObject(t1.process);
              t2 = A.Process_env(t2, os === "windows" ? "USERPROFILE" : "HOME");
              t2.toString;
              pubCache = A.join(t2, ".pub-cache");
              A._asJSObject(t1.core).exportVariable("DART_HOME", sdkPath);
              A._asJSObject(t1.core).addPath(A.join(sdkPath, "bin"));
              A._asJSObject(t1.core).exportVariable("PUB_CACHE", pubCache);
              A._asJSObject(t1.core).addPath(A.join(pubCache, "bin"));
              $async$goto = 20;
            ...
            case 20:
              ...
              if (A._asString(A._asJSObject(t1.core).getInput("problem-matcher")) !== "false") {
                ...
                if (actionPath.length === 0) {
                  actionPath = A._asStringQ(t1.actionRepoRoot);
                  if (actionPath == null)
                    actionPath = ".";
                }
                A.printString("::add-matcher::" + A.join(actionPath, "dart-analyzer.json"));
              }
              $async$goto = 21;
    ...
    getPlatform() {
      var t1,
        _0_0 = A._asString(A._asJSObject(init.G.os).platform());
      $label0$0: {
        if ("win32" === _0_0) {
          t1 = "windows";
          break $label0$0;
    ...
    createPubOIDCToken() {
      var $async$goto = 0,
        $async$completer = A._makeAsyncAwaitCompleter(type$.void),
        $async$returnValue, t2, t1, tokenRequestUrl, tokenRequestToken;
        ...
        for (;;)
          switch ($async$goto) {
            case 0:
              // Function start
              t1 = init.G;
              tokenRequestUrl = A.Process_env(A._asJSObject(t1.process), "ACTIONS_ID_TOKEN_REQUEST_URL");
              tokenRequestToken = A.Process_env(A._asJSObject(t1.process), "ACTIONS_ID_TOKEN_REQUEST_TOKEN");
              ...
              $async$goto = 3;
              return A._asyncAwait(A.promiseToFuture(A._asJSObject(A._asJSObject(t1.core).getIDToken("https://pub.dev")), type$.String), $async$createPubOIDCToken);
            case 3:
              // returning from await.
              t2 = $async$result;
              A._asJSObject(t1.core).exportVariable("PUB_TOKEN", t2);
              $async$goto = 4;
              return A._asyncAwait(A.promiseToFuture(A._asJSObject(A._asJSObject(t1.exec).exec("dart", A._setArrayType(["pub", "token", "add", "https://pub.dev", "--env-var", "PUB_TOKEN"], type$.JSArray_String))), type$.nullable_Object), $async$createPubOIDCToken);
            case 4:
              // returning from await.
            case 1:
              // return
        ...
        for (;;)
          switch ($async$goto) {
            ...
            case 3:
              ...
              t1 = A._arrayInstanceType(versions);
              t2 = t1._eval$1("MappedListIterable<1,Version>");
              semvers = A.List_List$_of(new A.MappedListIterable(versions, t1._eval$1("Version(1)")._as(A.version_Version___parse_tearOff$closure()), t2), t2._eval$1("ListIterable.E"));
    ...
    _fail(message) {
      var t1 = init.G;
      A._asJSObject(t1.core).error(message);
  ...
  A.IterableEquality.prototype = {
    equals$2(elements1, elements2) {
      ...
      for (t1 = t1._precomputed1, t2 = t2._precomputed1;;) {
  ...
  (function inheritance() {
    ...
    _inheritMany(A.Iterable, [A._CastIterableBase, A.EfficientLengthIterable, A.MappedIterable, A.WhereIterable, A.WhereTypeIterable, A._AllMatchesIterable, A._StringAllMatchesIterable]);
...
;
    ...
    B.List_x64_ia32_arm_arm64 = makeConstList(["x64", "ia32", "arm", "arm64"], type$.JSArray_String);
